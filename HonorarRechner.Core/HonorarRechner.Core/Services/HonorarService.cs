...
namespace HonorarRechner.Core.Services
{
    public class HonorarService
    {
        ...
        /// <summary>
        /// Berechnet eine einzelne Position aus der Liste der privaten Leistungen
        /// </summary>
        public void BerechnePrivatePosition(PrivatLeistung pos, TabellenWerte w)
        {
            if (pos == null) return;

            // Welcher Satz und welche Tabelle?
            decimal satz = 0;
            double volleGebuehr = 0;

            // Logik-Weiche anhand des Titels
            if (pos.Titel.Contains("Einkommensteuererklärung"))
            {
                // Tabelle A
                satz = w.Priv_EstSatz; // z.B. 3.5
                volleGebuehr = _rechner.BerechneVolleGebuehrBeratung((double)pos.Gegenstandswert);
            }
            else if (pos.Titel.Contains("Ermittlung des Überschusses") || pos.Titel.Contains("Gewerbebetrieb"))
            {
                // Tabelle B (Abschluss)
                satz = w.Priv_UeberschussSatz; // z.B. 10.0 (bei Tabelle B ist Faktor 1.0 = 10/10)
                // Achtung: Hier nutzen wir Tabelle B!
                volleGebuehr = _rechner.BerechneVolleGebuehrAbschluss((double)pos.Gegenstandswert);
            }
            else if (pos.Titel.Contains("USt-Erklärung"))
            {
                // Tabelle A
                satz = w.Priv_UstSatz;
                volleGebuehr = _rechner.BerechneVolleGebuehrBeratung((double)pos.Gegenstandswert);
            }
            else if (pos.Titel.Contains("Prüfung"))
            {
                // Tabelle A (Rat)
                satz = w.Priv_BescheidSatz;
                volleGebuehr = _rechner.BerechneVolleGebuehrBeratung((double)pos.Gegenstandswert);
            }
            else
            {
            ...
            // Ergebnis in die Zeile schreiben
            // Wir teilen den Satz durch 10, da die User-Eingabe z.B. "35" für 3,5/10 sein könnte oder "3.5".
            // Da wir oben "3.5" als Default definiert haben, rechnen wir direkt:
            // Tabelle A/C ist Dezimal (3.5/10). Tabelle B ist oft auch Dezimalfaktor (10/10 = 1.0).
            // Anpassung an deine Bruch-Logik:

            // Wenn Tabelle A (Beratung): Faktor ist z.B. 3.5 (für 3,5/10) -> Gebühr * 3.5 / 10
            // Wenn Tabelle B (Abschluss): Faktor ist z.B. 10 (für 10/10) -> Gebühr * 10 / 10?
            // Nein, dein GebuehrenRechner liefert "Volle Gebühr".
            // Tabelle A Volle Gebühr = 10/10. -> Satz 3.5 bedeutet 3.5 Zehntel. -> * 0.35

            // Wir machen es einheitlich: Satz ist immer "x Zehntel".
            pos.Honorar = (decimal)volleGebuehr * (satz / 10m);
        }

        public decimal BerechnePrivateGesamtsumme()
        {
            decimal summe = 0;
            foreach (var item in GlobalState.Instance.PrivateLeistungen)
            {
                summe += item.Honorar;
            }
            return summe;
